...
import { useWorldStateQuery } from '../../src/data/queries';
import type { WorldEntity } from '../../src/types';
...
export default function EntitiesPage() {
  const { data: world, error } = useWorldStateQuery();
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a1a', color: '#eee', fontFamily: 'monospace', padding: 24 }}>
      ...
      {error && <p style={{ color: '#ff5555' }}>fetch failed: {error.message}</p>}
      {allTypes.map((type) => {
